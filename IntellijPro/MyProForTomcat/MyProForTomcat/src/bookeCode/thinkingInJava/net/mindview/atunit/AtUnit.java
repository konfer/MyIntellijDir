...
public class AtUnit implements ProcessFiles.Strategy
{
	...
	public static void main(String[] args) throws Exception
	{
		...
		if(failures == 0) print("OK (" + testsRun + " tests)");
		else
		{
	...
	public void process(File cFile)
	{
		...
		for(Method m : testMethods)
		{
			...
			try
			{
				...
				try
				{
					if(m.getReturnType().equals(boolean.class)) success = (Boolean) m
							.invoke(testObject);
					else
					{
						m.invoke(testObject);
	...
	static class TestMethods extends ArrayList<Method>
	{
		void addIfTestMethod(Method m)
		{
			...
			if(!(m.getReturnType().equals(boolean.class) || m.getReturnType()
					.equals(void.class))) throw new RuntimeException(
	...
	private static Method checkForCreatorMethod(Method m)
	{
		...
		if(!m.getReturnType().equals(testClass)) throw new RuntimeException(
				"@TestObjectCreate "
						+ "must return instance of Class to be tested");
		if((m.getModifiers() & java.lang.reflect.Modifier.STATIC) < 1) throw new RuntimeException(
	...
	private static Method checkForCleanupMethod(Method m)
	{
		...
		if(!m.getReturnType().equals(void.class)) throw new RuntimeException(
				"@TestObjectCleanup " + "must return void");
		if((m.getModifiers() & java.lang.reflect.Modifier.STATIC) < 1) throw new RuntimeException(
				"@TestObjectCleanup " + "must be static.");
		if(m.getParameterTypes().length == 0
				|| m.getParameterTypes()[0] != testClass) throw new RuntimeException(
				"@TestObjectCleanup "
